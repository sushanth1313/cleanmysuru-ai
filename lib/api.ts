...
export const api = {
  ...
  async markWorkDone(id: string, formData: FormData) {
    const res = await fetch(`${API_BASE}/complaints/${id}/work-done`, {
      method: 'POST',
      headers: { ...getAuthHeaders() },
      credentials: 'include',
      body: formData,
  ...
  async getCitizenDashboard() {
    const res = await fetch(`${API_BASE}/dashboard/citizen`, {
      headers: { ...getAuthHeaders() },
      credentials: 'include',
    });
    return handleResponse<any>(res);
  ...
  async getMapComplaints(params: Record<string, string | number> = {}) {
    ...
    const res = await fetch(`${API_BASE}/map/complaints?${searchParams.toString()}`, {
      headers: { ...getAuthHeaders() },
      credentials: 'include',
    });
    return handleResponse<any>(res);
  ...
  async searchLocations(query: string) {
    ...
    const res = await fetch(`${API_BASE}/map/search?q=${encodeURIComponent(query.trim())}`, {
      headers: { ...getAuthHeaders() },
      credentials: 'include',
    });
  ...
  async reverseGeocode(lat: number, lng: number) {
    const res = await fetch(`${API_BASE}/map/reverse?lat=${lat}&lng=${lng}`, {
      headers: { ...getAuthHeaders() },
      credentials: 'include',
    });
  ...
  async getAuditLogs(params: Record<string, string | number> = {}) {
    ...
    const res = await fetch(`${API_BASE}/admin/audit-logs?${searchParams.toString()}`, {
      headers: { ...getAuthHeaders() },
      credentials: 'include',
    });
    return handleResponse<any>(res);
